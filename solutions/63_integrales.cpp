...
int pedir(double coef[N]){
    ...
    printf("Coef: ");
    gets(input); // gets es como un scanf lo unico k ha diferencia del scanf para cuando encuentra un un vacio .. este lee toda la linea///
    tok = strtok(input, ",");//Strtok coge el programa y lo separa por puntos y comas //
    if (tok)
	coef[n++] = atof(tok);
